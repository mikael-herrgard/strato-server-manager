...
"""
    ...
    def install_mailcow(self, domain: str, timezone: str = "America/New_York") -> bool:
        """
        Install Mailcow with domain configuration

        Args:
            domain: Primary domain for Mailcow (e.g., mail.example.com)
            timezone: Timezone for Mailcow (default: America/New_York)

        Returns:
            True if successful
        """
        logger.info(f"Starting Mailcow installation for domain: {domain}")

        mailcow_path = self.mailcow_config['install_path']

        # Check if already installed
        if os.path.exists(mailcow_path):
            logger.warning(f"Mailcow directory already exists: {mailcow_path}")
            return False
        ...
        if not prereqs['disk_space']:
            logger.error("Insufficient disk space (20GB required)")
            return False

        if not prereqs['ram']:
            logger.error("Insufficient RAM (4GB required)")
            return False
        ...
        try:
            with CommandExecutor("Installing Mailcow"):
                ...
                logger.info("Cloning Mailcow repository...")
                returncode, stdout, stderr = run_command(
                ...
                logger.info("Generating Mailcow configuration...")
                returncode, stdout, stderr = run_command(
                    ['./generate_config.sh'],
                    check=True,
                    cwd=mailcow_path,
                ...
                logger.info("Mailcow installation completed successfully")
                logger.info(f"Domain: {domain}")
                logger.info(f"Admin UI: https://{domain}")
                logger.info("Default login: admin / moohoo")
                logger.info("")
                logger.info("IMPORTANT: Configure DNS records:")
                logger.info(f"  A     {domain}           -> {self._get_public_ip()}")
                logger.info(f"  MX    example.com       -> {domain} (priority 10)")
                logger.info(f"  TXT   example.com       -> v=spf1 mx ~all")
                logger.info(f"  TXT   _dmarc.example.com -> v=DMARC1; p=quarantine; rua=mailto:postmaster@{domain}")
                logger.info("")
                logger.info("After DNS propagation, run: https://{domain}/admin for DKIM keys")

                return True

        except subprocess.CalledProcessError as e:
            logger.error(f"Mailcow installation failed: {e}")
            # Cleanup on failure
            if os.path.exists(mailcow_path):
                logger.info("Cleaning up failed installation...")
                try:
                    run_command(['docker', 'compose', 'down'], cwd=mailcow_path, check=False, timeout=120)
                    shutil.rmtree(mailcow_path)
                except Exception as cleanup_error:
                    logger.error(f"Cleanup failed: {cleanup_error}")
            return False
        except Exception as e:
            logger.error(f"Unexpected error during Mailcow installation: {e}")
            return False
